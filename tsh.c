 ...
 * 
 * Written by Chad Miller Nov. 2013 
 */
...
void eval(char *cmdline)
{
	...
	if (!builtin_cmd(argv)) {
		...
		if (!bg) {
			...
			Sigprocmask(SIG_UNBLOCK, &mask, NULL);
			printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);	//print bg job details  
		}
...
void sigint_handler(int sig)
{
	...
	if (fg_pid)
		Kill(-fg_pid, sig);	/*terminate fg process group */
	return;
}
...
/*Finds error in bg fg command.
 *If error returns NULL.*/
struct job_t *find_err(char **argv)
{
